...
def server_aggregation(num_clients, num_rounds, round):
    ...
    for client_id in range(num_clients):
        ...
        proto_data.extend(proto_labels.values())  # Gather all client protos for server clustering

    # Server uses AffinityPropagation to cluster client protos into server-level clusters
    server_sl_labels = apply_clustering(torch.stack(proto_data), method_name='AffinityPropagation')
    ...
    for client_id in range(num_clients):
        ...
        aligned_data = {server_label: [] for server_label in server_proto_representatives}
        ...
        train_model_by_server_labels(aligned_data, num_classes=len(server_proto_representatives))
