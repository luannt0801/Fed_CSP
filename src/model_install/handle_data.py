...
from src.add_config import data_config, logger_config, server_config
from src.model_install.model import BiLSTM, LeNet, LSTMModel
from src.model_install.setup import (
    mkdirs,
    print_dataset,
    processing_domain,
    read_yaml,
...
def get_Dataset(datasetname, datapath):
    ...
    if datasetname == "FashionMnist":
        ...
        dga_types = [
            dga_type
            for dga_type in os.listdir(data_folder)
            if os.path.isdir(f"{data_folder}/{dga_type}")
        ]
        # print(dga_types)
        ...
        for dga_type in dga_types:
            ...
            for file in files:
                with open(f"{data_folder}/{dga_type}/{file}", "r") as fp:
                    domains_with_type = [
                        [(line.strip()), dga_type, 1] for line in fp.readlines()
                    ]
                    appending_df = pd.DataFrame(
                        domains_with_type, columns=["domain", "type", "label"]
                    )
                    my_df = pd.concat([my_df, appending_df], ignore_index=True)

        with open(f"{data_folder}/benign.txt", "r") as fp:
            domains_with_type = [
                [(line.strip()), "benign", 0] for line in fp.readlines()[:]
            ]  # read all file
            appending_df = pd.DataFrame(
                domains_with_type, columns=["domain", "type", "label"]
            )
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            padded_domains, encoded_labels, test_size=0.10, shuffle=True
        )

        trainset = TensorDataset(
            torch.tensor(X_train, dtype=torch.long), torch.Tensor(y_train)
        )
        testset = TensorDataset(
            torch.tensor(X_test, dtype=torch.long), torch.Tensor(y_test)
        )
        ...
        for dga_type in dga_types:
            ...
            for file in files:
                ...
                with open(f"{data_folder}/{dga_type}/{file}", "r") as fp:
                    domains_with_type = [
                        [(line.strip()), dga_type, current_label]
...
def split_data(dataset_use, **kwargs):
    ...
    if "num_client" not in kwargs:
        num_client = 10
        warnings.warn("num_client is not being included. set num_client = 10")
    else:
        num_client = kwargs["num_client"]

    if "partition" not in kwargs:
        partition = "iid_equal_size"
        warnings.warn("partition: iid_equal_size")
    else:
    ...
    if "data_for_client" not in kwargs:
        ...
        warnings.warn("data_client is saved in folder: sperated_data_client")
    else:
    ...
    #         # print(f"Client {cid} data saved to {client_save_file}")
        ...
        if data_volume_each_client == "unequal":
            ...
            raise ValueError(
                "Don't know how to distribute data volume across clients!!"
            )
        ...
        for cid in range(num_client):
            chosen_classes = np.random.choice(
                num_classes, np.random.randint(2, 4), replace=False
            )
            for class_idx in chosen_classes:
                if data_volume_each_client == "unequal":
                    ...
                    num_samples = int(
                        proportions.sum() * len(label_to_indices[class_idx])
                    )
                    client_indices[cid].extend(
                        label_to_indices[class_idx][:num_samples]
                    )
                    label_to_indices[class_idx] = label_to_indices[class_idx][
                        num_samples:
                    ]
                elif data_volume_each_client == "equal":
