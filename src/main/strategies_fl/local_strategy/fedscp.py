from src.main.strategies_fl.local_strategy.cluster_fuc import do_silhouette_score, apply_clustering
...
def update_trainset_with_new_labels(all_client_trainset, final_mapping):
    ...
    for client_id, trainset in all_client_trainset.items():
        ...
        if len(new_labels) != len(reality_labels):
            raise ValueError(f"The number of new labels does not match the number of original client {client_id} labels.")
...
def local_fedscp(num_clients, all_client_trainset,all_client_testset, client_res_dict):
    ...
    """
    finnal_mapping = cluster_by_data(all_client_trainset,all_client_testset, num_clients)

    # for key, value in finnal_mapping.items():
    #     print(f"key in final_mapping: {key}")
    #     print(f"value in final_mapping: {len(value)}")

    updata_all_trainset_client = update_trainset_with_new_labels(all_client_trainset=all_client_trainset, final_mapping=finnal_mapping)
